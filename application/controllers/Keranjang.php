...
class Keranjang extends CI_Controller
{
  ...
  {
    $data['judul'] = "PAKU | Keranjang Saya";
  ...
  public function uploadTransfer($id)
  {
    ...
    if ($upload) {
      ...
      if ($this->upload->do_upload('bukti-tf')) {
        ...
        $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-pesan col">Terima Kasih telah memesan, silahkan tunggu no telpon yang dapat di hubungi.</div>');
        redirect('profile');
      } else {
  ...
  public function updateStatusPengiriman()
  {
    ...
    $riwayat = $produk['riwayat_kerja'];
    $riwayat += 1;


    $this->db->set('riwayat_kerja', $riwayat);
    $this->db->where('id_produk', $idProduk);
    $this->db->update('produk');

    $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-pesan col">Terima Kasih telah mempercayai kuli kami.</div>');
    redirect('profile');
  }
