...
$(document).ready(function () {
	...
	$('.btn-tambah').click(function (e) {
		...
		$('form').attr('action', 'http://localhost/paku/produk/kategori');
		$('#nama_kategori').val('');
	...
	$('.edit-kategori').on('click', function (e) {
		...
		$('form').attr('action', 'http://localhost/paku/produk/editKategori/' + id);

		$.ajax({
			type: "post",
			url: "http://localhost/paku/produk/getKategori",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.hapus-kategori').click(function (e) {
		...
		const href = 'http://localhost/paku/produk/hapuskategori/' + id;
	...
	$('.hapus-produk').click(function (e) {
		...
		const href = 'http://localhost/paku/produk/hapusproduk/' + id;
	...
	$('.tb-diskon').click(function (e) {
		...
		$('form').attr('action', 'http://localhost/paku/produk/produkdiskon');
		$('#diskon').val('');
		$('.btn-toggle').html('Tambah');
	...
	$('.edit-diskon').on('click', function (e) {
		...
		$('form').attr('action', 'http://localhost/paku/produk/editDiskon/' + id);

		$.ajax({
			type: "post",
			url: "http://localhost/paku/produk/getDiskon",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.hapus-diskon').click(function (e) {
		...
		const href = 'http://localhost/paku/produk/hapusDiskon/' + id;
	...
	$('.tambah-bank').click(function (e) {
		...
		$('.form-bank').attr('action', 'http://localhost/paku/profile/tambahbank');
	...
	$('.edit-bank').on('click', function (e) {
		...
		$('.form-bank').attr('action', 'http://localhost/paku/profile/editbank/' + id);

		$.ajax({
			type: "post",
			url: "http://localhost/paku/profile/getBank",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.hapusBank').click(function (e) {
		...
		const href = 'http://localhost/paku/profile/hapusBank/' + id;
	...
	$('#konfirmasi-alamat').on('change', function () {
		...
		$.ajax({
			type: "post",
			url: "http://localhost/paku/keranjang/getAlamat",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.tambah-alamat').click(function (e) {
		...
		$('.form-alamat').attr('action', 'http://localhost/paku/profile/addAlamat');
	...
	$('.edit-alamat').on('click', function (e) {
		...
		$('.form-alamat').attr('action', 'http://localhost/paku/profile/editalamat/' + id);

		$.ajax({
			type: "post",
			url: "http://localhost/paku/profile/getAlamat",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.delete-alamat').click(function (e) {
		...
		const href = 'http://localhost/paku/profile/hapusAlamat/' + id;
	...
	$('.ambil-id').click(function (e) {
		...
		const href = 'http://localhost/paku/keranjang/uploadTransfer/' + id;
	...
	$('.kategori-produk').on('click', function (e) {
		...
		$.ajax({
			type: "post",
			url: "http://localhost/paku/produk/getProduk",
			data: {
				id: id
			},
			dataType: "json",
			success: function (response) {
				...
				for (let i = 0; i < response.length; i++) {
					...
					let cardRow = '<div class="col-md-4 col-6"><div class="card shadow w-80" style="margin-bottom:2.5rem"><div class="card-body"><div class="d-flex justify-content-center"><img src="http://localhost/paku/assets/img/produk/' + response[i].foto_produk + '" alt="" class="img-fluid mb-4 shadow"></div><div class="text-center"><h3>' + response[i].nama_produk + '</h3><p>Rp. ' + response[i].harga + ' </p></div><div class="d-flex justify-content-center"><a href="http://localhost/paku/produk/dProduk/' + response[i].id_produk + '" class="btn btn-cart-produk btn-dark">Lihat</a></div></div></div></div >';
	...
	$('.button-resi').click(function (e) {
		...
		$.ajax({
			type: "post",
			url: "http://localhost/paku/pesanan/getResi",
			data: {
				id: id
			},
			dataType: "json",
	...
	$('.edit-resi').click(function (e) {
		...
		$('.form-resi').attr('action', 'http://localhost/paku/pesanan/updateResi');
		...
		$.ajax({
			type: "post",
			url: "http://localhost/paku/pesanan/updateResi",
			data: {
				idPesanan: idPesanan,
				resiPengiriman: resi
			},
